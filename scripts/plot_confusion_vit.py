...
from model_vit import LipReadingViTModel as LipReadingModel
...
with torch.no_grad():
    for batch_idx, batch in enumerate(dataloader):
        ...
        for i, target_len in enumerate(target_lengths):
            ...
            if len(true_flat) == 0:
                print(f"🚨 Empty target – sample {batch_idx * batch_size + i}")
                continue

            if len(pred_flat) < 2:
                print(f"⚠️ Suspiciously short prediction – sample {batch_idx * batch_size + i} (len={len(pred_flat)})")
